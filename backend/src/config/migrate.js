...
const migrate = async () => {
  ...
  try {
    ...
    // 0. DROP EXISTING TABLES (Reset)
    await client.query(`DROP TABLE IF EXISTS complaints CASCADE;`);
    await client.query(`DROP TABLE IF EXISTS relocation_requests CASCADE;`);
    await client.query(`DROP TABLE IF EXISTS reviews CASCADE;`);

    await client.query(`DROP TABLE IF EXISTS lease_requests CASCADE;`);
    ...
    await client.query(`DROP TABLE IF EXISTS landlords CASCADE;`);
    ...
    await client.query(`CREATE INDEX idx_units_property ON units(property_id);`);
    await client.query(`CREATE INDEX idx_units_status ON units(status);`);
    await client.query(`CREATE INDEX idx_leases_tenant ON leases(tenant_id);`);
    await client.query(`CREATE INDEX idx_leases_unit ON leases(unit_id);`);
    ...
    await client.query(`CREATE INDEX idx_lease_requests_unit ON lease_requests(unit_id);`);
    await client.query(`CREATE INDEX idx_lease_requests_tenant ON lease_requests(tenant_id);`);
    await client.query(`CREATE INDEX idx_reviews_property ON reviews(property_id);`);


    await client.query('COMMIT');
    console.log('✅ Database schema reset and migration completed successfully!');
  } catch (err) {
